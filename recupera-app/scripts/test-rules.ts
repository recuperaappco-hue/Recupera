...
t("bank alert + merchant receipt for one payment is not a duplicate", () => {
  const { findings } = buildFindings([
    E({ gmail_id: "b1", kind: "charge", merchant: "CENS", amount: 1076357, sender: "Bancolombia <alertasynotificaciones@an.notificacionesbancolombia.com>", received_at: "2026-09-10T15:00:00.000Z" }),
    E({ gmail_id: "b2", kind: "charge", merchant: "CENS", amount: 1076357, sender: "PSE <notificaciones@pse.com.co>", received_at: "2026-09-10T15:01:00.000Z" }),
  ], TODAY);
  assert.equal(findings.length, 0);
});

t("habitual merchant with same amount on many days is not flagged", () => {
  const C = (id: string, at: string) => E({ gmail_id: id, kind: "charge", merchant: "Ruta 81 Carwash", amount: 20000, received_at: at });
  const { findings } = buildFindings([
    C("w1", "2026-09-02T10:00:00.000Z"), C("w2", "2026-09-02T10:05:00.000Z"),
    C("w3", "2026-09-10T10:00:00.000Z"), C("w4", "2026-09-20T10:00:00.000Z"),
  ], TODAY);
  assert.equal(findings.length, 0);
});
